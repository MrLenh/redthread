...
update_table_status = ["In-Production", "Done"] #"Shipped"
def get_table_data(base_id, table_id, token):
    ...
    while has_more:
       ...
       #order_list = get_order_list(orderData)
        ...
        page += 1
    with open("order_data1.json", "w", encoding="utf-8") as f:
...
def extract_orders_from_json(json_data, preview = False):
    ...
    for item in data.get('items', []):
        ...
        if("Color Index" in item["fields"]): 
            order["mockup_token"] = item["fields"]["Color Index"][0]["file_token"]
...
def download_order_artworks(orders):
    ...
    for order in orders:
        ...
        if order.get("mockup") is not None:
            ...
            file_token = getOrderID(None, record_id)[0]["mockup_token"]
...
def update_status_from_qr(record_id):
    try:
        ...
        if request.method == 'GET':
            print("✅[Debug] get records.html : ", order_id, order_data, record_id, preview_url, ROLE_USER)
            return render_template("records.html", order_id=order_id, records=order_data, active_record_id=record_id, design_link=preview_url, roles =ROLE_USER)
        # POST -> JSON success
