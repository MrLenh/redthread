...
def get_table_data(base_id, table_id, token):
    ...
    while has_more:
        ...
        upsert_local_records(orderData, "order_data.json", False)
...
def get_orders_records(record_id, order_id):
    ...
    if local_data :
        for record in local_data:
            print("✅[get_orders_records] order_id:", record["order_id"])
            if record["order_id"] == order_id:
                print("✅[get_orders_records] record:", record)
                order_data.append(record)
                if record["record_id"] == record_id and record["status"] == "In_Production" :
        ...
        if order_data:
            ...
            print("✅[get_orders_records] order_data:", order_data)
            return order_data
...
def process_orders():
    ...
    try:
        ...
        return None
    #download_order_artworks(processing_data)
    #download_order_artworks(data)
...
def create_qr_labels(orders):
    ...
    for idx, order in enumerate(orders):
        if order.get("mockup") is not None:
            get_lark_file_preview_url(order["mockup"], order["record_id"])
...
def extract_orders_from_json(json_data, preview = False):
    ...
    for item in data.get('items', []):
        ...
        if("Color Index" in item["fields"]): 
            order["mockup"] = item["fields"]["Color Index"][0]["file_token"]
        ...
        if(order["shop_name"].find("PGS") != -1): 
            ...
            order["mockup"] =  item["fields"]["Mockup"][0]["file_token"]
...
def download_order_artworks(orders):
    ...
    for order in orders:
        ...
        if order.get("mockup") is not None:
            preview_url = get_lark_file_preview_url(order["mockup"],order.get("record_id"))
        else:
...
def update_status_from_qr(record_id):
    try:
        ...
        if os.path.exists(f"templates/static/tmp/{record_id}"):
            ...
            order = next((item for item in recordData if item["record_id"] == record_id), None)
            preview_url = download_order_artworks([order]) 
...
def update_record_data_json(record_id, status):
    ...
    for record in local_data:
        if record["record_id"] == record_id:
            record["record_id"]['status'] = status
            print("✅[update_record_data_json] status:", status)
    with open("order_data.json", "w", encoding="utf-8") as f:
            json.dump(local_data, f, ensure_ascii=False, indent=4)

def update_record_status(record_id, status):
    current_status = get_record_data_json(record_id)['status']
    print("✅[update_record_status] first current_status:", current_status, status)
    if not get_next_status(current_status, status) :
        return None
    try:
        current_status = getOrderID(None, record_id)[0]['status']
        print("✅[update_record_status] current_status:", current_status)
