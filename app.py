...
def get_table_data(base_id, table_id, token):
    ...
    while has_more:
        ...
        upsert_local_records(order_list, "order_data.json", False)
        ...
        for record in local_data:
           # print("✅[get_orders_records] order_id:", record["order_id"])
            if record["order_id"] == order_id :
                if record["record_id"] == record_id and record["status"] == "In-Production" :
...
def update_record_status(record_id, status):
    ...
    with app.app_context():
        ...
        try:
            ...
            print("✅[update_record_status] Update from -> to:", current_status, status)
            user_token = get_access_token(APP_ID, APP_SECRET)
            ...
            lark.logger.error(f"Exception updating record {record_id}: {e}, 500")
            return record_id
    return record_id
