...
def get_table_data(base_id, table_id, token):
    ...
    while has_more:
        ...
        orderData = extract_orders_from_json(response_data, True)
...
def get_orders_records(record_id):
    with open("order_data.json", "r", encoding="utf-8") as f:
        local_data = json.load(f)

    for record in local_data:
        if record["record_id"] == record_id:
            print("✅[get_orders_records] Found record_id:", record_id)
            return record
    data = getOrderID(None, record_id)
    local_data.append(data[0])
    with open("order_data.json", "w", encoding="utf-8") as f:
...
def getOrderID(order_id, record_id=None):
    ...
    # send request
    if record_id:
        print("✅[getOrderID] Get record_id:", record_id)
        request: SearchAppTableRecordRequest = SearchAppTableRecordRequest.builder() \
        .app_token(BASE_ID) \
        .table_id(TABLE_ID) \
        .user_id_type("user_id") \
        ...
        print("✅[getOrderID] Get orders:", order_id)
        request: SearchAppTableRecordRequest = SearchAppTableRecordRequest.builder() \
            .app_token(BASE_ID) \
            .table_id(TABLE_ID) \
            .user_id_type("user_id") \
    ...
    print("✅[getOrderID] orderData: ", orderData)
    return orderData
...
def update_status_from_qr(record_id):
    try:
        ...
        recordData = get_orders_records(record_id)
        ...
        if request.method == 'GET':
            ...
            return render_template("orders.html", order_id=order_id, records=[order_data], active_record_id=record_id, design_link=preview_url, roles =ROLE_USER)
        # POST -> JSON success
...
def download_png(file_id, record_id):
    ...
    os.makedirs(folder, exist_ok=True)
    filepath = os.path.join(folder, record_id)
    if os.path.exists(f"static/tmp/{record_id}"):
        print("✅ File already exists locally:", record_id)
        return filepath
